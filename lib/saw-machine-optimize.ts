...
export interface OptimizeResult {
  ...
  sawSchedule: SawScheduleEntry[]
  applied: boolean
  message?: string
}

export async function runSawMachineOptimize(params: {
  date: string
  phase: number
  dryRun: boolean
}): Promise<OptimizeResult> {
  const { date, phase, dryRun } = params
  const phaseStr = String(phase)
  const period = PERIOD[phaseStr]
  if (!date || !period) return { changes: [], sawSchedule: [], applied: false, message: 'missing params' }
  ...
  const sawStationRate = new Map<string, number>()
  const sawNormSkuSet  = new Set<string>()
  ...
  if (!sawStationRate.size) return { changes: [], sawSchedule: [], applied: false, message: 'ไม่มีข้อมูล mas_saw_machine_sku' }
  ...
  if (!assignments.length) return { changes: [], sawSchedule: [], applied: false, message: 'ไม่มีข้อมูลคำสั่งผลิต' }
  ...
  return { changes, sawSchedule, applied: !dryRun && changes.length > 0 }
}
