...
const STATIONS = ['ทั้งหมด', 'สะโพก', 'สามชั้น', 'ไหล่', 'หมูบด', 'สไลด์', 'เผาขา', 'เลาะขา']
...
const STATION_COLOR: Record<string, { border: string; bg: string; text: string; dot: string }> = {
  ...
  'สะโพก':   { border: 'border-orange-400',  bg: 'bg-orange-50',  text: 'text-orange-700',  dot: 'bg-orange-400' },
  'สามชั้น': { border: 'border-blue-400',    bg: 'bg-blue-50',    text: 'text-blue-700',    dot: 'bg-blue-400' },
  'ไหล่':    { border: 'border-green-400',   bg: 'bg-green-50',   text: 'text-green-700',   dot: 'bg-green-400' },
  'หมูบด':   { border: 'border-red-400',     bg: 'bg-red-50',     text: 'text-red-700',     dot: 'bg-red-400' },
  'สไลด์':   { border: 'border-purple-400',  bg: 'bg-purple-50',  text: 'text-purple-700',  dot: 'bg-purple-400' },
  'เผาขา':   { border: 'border-fuchsia-400', bg: 'bg-fuchsia-50', text: 'text-fuchsia-700', dot: 'bg-fuchsia-400' },
  'เลาะขา':  { border: 'border-teal-400',    bg: 'bg-teal-50',    text: 'text-teal-700',    dot: 'bg-teal-400' },
}
...
export default function BreaklinePage() {
  ...
  const fetchBreaks = useCallback(async () => {
    setLoading(true)
    try {
      const res  = await fetch(`/api/line-break?date=${date}`)
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res  = await fetch('/api/line-break', {
        method: 'POST',
  ...
  const handleDelete = async (id: string) => {
    setDeletingId(id)
    try {
      await fetch(`/api/line-break?id=${id}`, { method: 'DELETE' })
      fetchBreaks()
