...
function GroupCard({ group }: { group: AllocationGroup }) {
  const cfg = PRIORITY_CONFIG[group.priority] ?? PRIORITY_CONFIG[2]
  const totalNeeded = group.items.reduce((s, i) => s + i.needed_kg, 0)
  const totalAllocated = group.items.reduce((s, i) => s + i.allocated_kg, 0)
  const totalShortage = group.items.reduce((s, i) => s + i.shortage_kg, 0)
  const hasShortage = totalShortage > 0.5
      ...
      <div className="flex items-center justify-between mb-4">
        ...
        {hasShortage && (
          <span className="text-xs text-red-600 font-medium bg-red-50 px-2 py-0.5 rounded-full shrink-0">
            ขาด {Math.round(totalShortage).toLocaleString()} กก.
          </span>
        )}
      </div>
          ...
          <table className="w-full text-sm">
            ...
            <tfoot className="border-t-2 border-gray-200 bg-gray-50">
              <tr>
                ...
                <td className={`px-3 py-2 text-right text-sm font-bold tabular-nums ${hasShortage ? 'text-red-600' : 'text-green-600'}`}>{Math.round(totalAllocated).toLocaleString()}</td>
                <td className={`px-3 py-2 text-right text-sm font-bold hidden sm:table-cell tabular-nums ${hasShortage ? 'text-red-500' : 'text-gray-400'}`}>
                  {hasShortage ? Math.round(totalShortage).toLocaleString() : '—'}
                </td>
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        <div>
          ...
          <h1 className="text-xl md:text-2xl font-bold text-gray-900">จัดสรรเนื้อ Raw Mat — Phase 1</h1>
          <p className="text-gray-500 mt-1 text-sm hidden sm:block">แบ่งวัตถุดิบไปแต่ละ Station ตาม Priority (ตาม Mas Priority เบิก RM)</p>
        </div>
      </div>
      ...
      {!loading && result && !result.message && (
        <div className="space-y-6">
          ...
          <div className="card">
            ...
            >
              <div>
                <h2 className="font-semibold text-gray-900">สรุปวัตถุดิบรวม</h2>
                <p className="text-xs text-gray-500 mt-0.5">
                  Stock รวม {Math.round(totalRaw).toLocaleString()} กก. · จัดสรร {Math.round(totalAllocated).toLocaleString()} กก.
                </p>
              </div>
          ...
          {/* Priority breakdown */}
          <div>
            <h2 className="font-semibold text-gray-700 text-sm mb-3">การจัดสรรตาม Priority</h2>
            <div className="space-y-4">
              {result.allocation.map((group, i) => (
