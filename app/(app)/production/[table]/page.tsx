...
const CFG: Record<string, { label: string; accent: string; light: string }> = {
  ...
  'pao-kha':   { label: 'เผาขา',  accent: 'border-fuchsia-500', light: 'bg-fuchsia-50' },
  'loa-kha':   { label: 'เลาะขา', accent: 'border-teal-500',    light: 'bg-teal-50'    },
}
                ...
                <div className="flex items-start gap-1.5 sm:gap-2">
                  ...
                  <div className="min-w-0">
                    ...
                    >
                      ...
                      {ackedSkus.has(sku) ? 'รับทราบ ✓' : 'รับทราบ'}
                    </button>
                  </div>
                </div>
              </div>
                ...
                {mergeSegmentsWithWorkers(stat.segments.filter(s => !s.isDeficit)).map((seg, idx) => {
                  ...
                  return (
                    <div key={`s-${idx}`} className="absolute top-2 bottom-2 sm:top-2.5 sm:bottom-2.5 rounded-sm cursor-pointer"
                      style={{ left: `${barLeft}%`, width: `${barWidth}%`, backgroundColor: col.bg, opacity }}
...
export default function TablePage() {
  ...
  const generateSupplementary = async (slot: number) => {
    setGenSupSlot(slot); setGenSupResult(null)
    try {
      const res = await fetch('/api/production/generate-supplementary', {
        method: 'POST', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ date, slot }),
      })
      const result = await res.json()
      setGenSupResult(result)
      if (result.success) loadData(date)
    } catch { setGenSupResult({ success: false, message: 'เกิดข้อผิดพลาด' }) }
    setGenSupSlot(null)
  }
