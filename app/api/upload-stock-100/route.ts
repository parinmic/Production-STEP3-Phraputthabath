...
export async function POST(req: NextRequest) {
  try {
    ...
    await supabase.from('upload_log').delete().eq('table_name', 'stock_100')

    const uploadLogId = crypto.randomUUID()
    const { error: logErr } = await supabase
      .from('upload_log')
      .insert({ id: uploadLogId, table_name: 'stock_100', source_file: filename ?? 'unknown', record_count: records.length })
    if (logErr) throw logErr

    const recordsWithId = records.map((r: Record<string, unknown>) => ({ ...r, upload_log_id: uploadLogId }))
    const { error } = await supabase.from('stock_100').insert(recordsWithId)
    if (error) {
      await supabase.from('upload_log').delete().eq('id', uploadLogId)
      throw error
    }

    await syncUploadToDev('stock_100', filename ?? 'unknown', records, true)
...
export async function DELETE(req: NextRequest) {
  try {
    ...
    // ON DELETE CASCADE removes stock_100 rows automatically
    await supabase.from('upload_log').delete().eq('id', uploadLogId)
