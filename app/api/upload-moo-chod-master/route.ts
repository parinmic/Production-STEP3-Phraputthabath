...
export async function POST(req: NextRequest) {
  try {
    ...
    const { error: logErr } = await supabase
      .from('upload_log')
      .insert({ id: uploadLogId, table_name: 'moo_chod_master', source_file: filename ?? 'unknown', record_count: records.length })
    ...
    await syncUploadToDev('moo_chod_master', filename ?? 'unknown', records)
