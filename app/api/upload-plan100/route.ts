...
export async function POST(req: NextRequest) {
  try {
    ...
    if (planDate) {
      try {
        const genMsg = await autoGeneratePhase3(planDate)
        return NextResponse.json({ success: true, message: `บันทึกสำเร็จ ${records.length} รายการ (วันที่ ${planDate}) · สร้างแผน Phase 3 อัตโนมัติ: ${genMsg}`, autoGenerated: true })
      } catch (genErr) {
        return NextResponse.json({ success: true, message: `บันทึกสำเร็จ ${records.length} รายการ (วันที่ ${planDate}) (Phase 3 auto-gen ล้มเหลว: ${genErr instanceof Error ? genErr.message : 'error'})` })
      }
    }
