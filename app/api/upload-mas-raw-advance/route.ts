...
export async function POST(req: NextRequest) {
  try {
    ...
    const { error: logErr } = await supabase
      .from('upload_log')
      .insert({ id: uploadLogId, table_name: 'mas_raw_production_advance', source_file: filename ?? 'unknown', record_count: records.length })
    ...
    await syncUploadToDev('mas_raw_production_advance', filename ?? 'unknown', records)
