...
export async function POST(req: NextRequest) {
  try {
    ...
    const { data: logEntry, error: logErr } = await supabase
      .from('upload_log')
      .insert({ table_name: 'mas_raw_production_advance', source_file: filename ?? 'unknown', record_count: records.length })
      .select('id')
      .single()
    if (logErr) throw logErr

    const recordsWithId = records.map((r: Record<string, unknown>) => ({ ...r, upload_log_id: logEntry.id }))
    const { error } = await supabase.from('mas_raw_production_advance').insert(recordsWithId)
    if (error) {
    ...
    await syncToDevAwaited(async (dev) => {
      ...
      await batchInsert(dev, 'mas_raw_production_advance', records.map((r: Record<string, unknown>) => ({ ...r, upload_log_id: devLog.id })))
    })
...
export async function DELETE(req: NextRequest) {
  try {
    ...
    // ON DELETE CASCADE removes mas_raw_production_advance rows automatically
