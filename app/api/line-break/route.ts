...
export async function GET(req: NextRequest) {
  ...
  let q = supabase.from('line_breaks').select('*').eq('production_date', date).order('start_time')
...
export async function POST(req: NextRequest) {
  try {
    ...
    const { data, error } = await supabase.from('line_breaks').insert({
...
export async function DELETE(req: NextRequest) {
  ...
  const { error } = await supabase.from('line_breaks').delete().eq('id', id)
