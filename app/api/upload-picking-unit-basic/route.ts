...
export async function POST(req: NextRequest) {
  try {
    ...
    await supabase.from('picking_unit_master_basic').delete().gte('id', 0)

    const { error } = await supabase.from('picking_unit_master_basic').insert(records)
    if (error) throw error

    await supabase.from('upload_log').delete().eq('table_name', 'picking_unit_master_basic')
    await supabase.from('upload_log').insert({
      table_name:   'picking_unit_master_basic',
      source_file:  filename ?? 'unknown',
      record_count: records.length,
    })
    await syncToDevAwaited(async (dev) => {
      await dev.from('picking_unit_master_basic').delete().gte('id', 0)
      await batchInsert(dev, 'picking_unit_master_basic', records)
    })
...
export async function DELETE(req: NextRequest) {
  try {
    ...
    await supabase.from('picking_unit_master_basic').delete().gte('id', 1)
    await supabase.from('upload_log').delete().eq('table_name', 'picking_unit_master_basic').eq('source_file', sourceFile)
    await syncToDevAwaited(async (dev) => {
      await dev.from('picking_unit_master_basic').delete().gte('id', 1)
    })
