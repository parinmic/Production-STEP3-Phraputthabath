  ...
  try {
    ...
    const records = rows
      .filter((r: Record<string, unknown>) => {
        ...
        if (bst === '924' || oper === '4903') return true
        return false
      })
    ...
    if (deliveryDates.length) {
      await supabase.from('fs_orders').delete().in('delivery_date', deliveryDates)
    }

    const { error } = await supabase.from('fs_orders').insert(records)
    if (error) throw error

    await supabase.from('upload_log').insert({
      table_name:   'fs_orders',
      source_file:  filename ?? 'unknown',
    ...
    syncToDev(async (dev) => {
      if (deliveryDates.length) {
        await dev.from('fs_orders').delete().in('delivery_date', deliveryDates)
      }
      await batchInsert(dev, 'fs_orders', records)
    })
...
export async function DELETE(req: NextRequest) {
  try {
    ...
    await supabase.from('fs_orders').delete().eq('source_file', sourceFile)
    await supabase.from('upload_log').delete().eq('table_name', 'fs_orders').eq('source_file', sourceFile)
