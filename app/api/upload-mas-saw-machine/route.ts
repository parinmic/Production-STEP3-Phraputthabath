...
export async function POST(req: NextRequest) {
  try {
    ...
    const { error: logErr } = await supabase
      .from('upload_log')
      .insert({ id: uploadLogId, table_name: 'mas_saw_machine_sku', source_file: filename ?? 'unknown', record_count: records.length })
    ...
    await syncUploadToDev('mas_saw_machine_sku', filename ?? 'unknown', records)
