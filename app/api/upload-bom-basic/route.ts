...
export async function POST(req: NextRequest) {
  try {
    ...
    const { error: logErr } = await supabase
      .from('upload_log')
      .insert({ id: uploadLogId, table_name: 'bom_items_basic', source_file: filename ?? 'unknown', record_count: records.length })
    if (logErr) throw logErr

    const recordsWithId = records.map((r: Record<string, unknown>) => ({ ...r, upload_log_id: uploadLogId }))
    const { error } = await supabase.from('bom_items_basic').insert(recordsWithId)
    if (error) {
      await supabase.from('upload_log').delete().eq('id', uploadLogId)
      throw error
    }

    await syncUploadToDev('bom_items_basic', filename ?? 'unknown', records)
...
export async function DELETE(req: NextRequest) {
  try {
    ...
    // ON DELETE CASCADE removes bom_items_basic rows automatically
    await supabase.from('upload_log').delete().eq('id', uploadLogId)
