...
export async function POST(req: NextRequest) {
  try {
    ...
    await supabase.from('bom_items_basic').delete().gte('id', 1)

    const { error } = await supabase.from('bom_items_basic').insert(records)
    if (error) throw error

    await supabase.from('upload_log').delete().eq('table_name', 'bom_items_basic')
    await supabase.from('upload_log').insert({
      table_name:   'bom_items_basic',
      source_file:  filename ?? 'unknown',
      record_count: records.length,
    })
    syncToDev(async (dev) => {
      await dev.from('bom_items_basic').delete().gte('id', 1)
      await batchInsert(dev, 'bom_items_basic', records)
    })
...
export async function DELETE(req: NextRequest) {
  try {
    ...
    await supabase.from('bom_items_basic').delete().gte('id', 1)
    await supabase.from('upload_log').delete().eq('table_name', 'bom_items_basic').eq('source_file', sourceFile)
    syncToDev(async (dev) => {
      await dev.from('bom_items_basic').delete().gte('id', 1)
    })
