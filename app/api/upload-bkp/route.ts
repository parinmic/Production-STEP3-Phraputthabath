...
export async function POST(req: NextRequest) {
  try {
    ...
    const { error: logErr } = await supabase
      .from('upload_log')
      .insert({ id: uploadLogId, table_name: 'bkp_orders', source_file: filename ?? 'unknown', record_count: records.length })
    if (logErr) throw logErr

    const recordsWithId = records.map((r: BkpRecord) => ({ ...r, upload_log_id: uploadLogId }))
    ...
    await syncUploadToDev('bkp_orders', filename ?? 'unknown', records)
