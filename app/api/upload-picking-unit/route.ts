...
export async function POST(req: NextRequest) {
  try {
    ...
    const { error: logErr } = await supabase
      .from('upload_log')
      .insert({ id: uploadLogId, table_name: 'picking_unit_master', source_file: filename ?? 'unknown', record_count: records.length })
    ...
    await syncUploadToDev('picking_unit_master', filename ?? 'unknown', records)
