          ...
          <div className="divide-y divide-gray-100 max-h-40 overflow-y-auto pr-1">
            {history.map((h, i) => (
              <div key={i} className="flex items-center gap-2 py-2 hover:bg-gray-50 rounded px-1">
                ...
                <button onClick={() => handleDelete(h.source_file)}
...
const themes = {
  ...
  'perd-moo': {
    border: 'border-t-4 border-slate-500',
...
export default function BasicWeeklyWorkforcePage() {
  return (
    <div className="space-y-6 md:space-y-8">
      <div>
        <h1 className="text-xl md:text-2xl font-bold text-gray-900">แผนเข้างานประจำสัปดาห์ (เบสิค)</h1>
        <p className="text-xs md:text-sm text-gray-500 mt-1">อัปโหลดตารางเข้างานประจำสัปดาห์ของพนักงานแต่ละกลุ่มงาน</p>
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <WeeklyUploader type="sa-phok-basic"  label="สะโพกเบสิค"   theme={themes['sa-phok-basic']} />
        <WeeklyUploader type="sam-chan-basic"  label="สามชั้นเบสิค" theme={themes['sam-chan-basic']} />
        <WeeklyUploader type="lai-basic"       label="ไหล่เบสิค"    theme={themes['lai-basic']} />
      </div>

      <div>
        <h2 className="text-base font-semibold text-gray-700 mb-4">สถานีเปิดหมู</h2>
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          <WeeklyUploader type="perd-moo" label="เปิดหมู" theme={themes['perd-moo']} />
        </div>
      </div>
    </div>
  )
