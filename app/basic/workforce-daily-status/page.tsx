...
const stations = [
  { value: 'sa-phok-basic',  label: 'สะโพกเบสิค' },
  { value: 'sam-chan-basic',  label: 'สามชั้นเบสิค' },
  { value: 'lai-basic',      label: 'ไหล่เบสิค' },
]
...
export default function BasicWorkforceDailyStatusPage() {
  ...
  const [selectedStation, setSelectedStation] = useState('sa-phok-basic')
  ...
  const getFieldValue = (rowData: Record<string, any>, prefixes: string[]): string => {
    ...
    for (const prefix of prefixes) {
      if (rowData[prefix] != null) return String(rowData[prefix]).trim()
    }
    const keys = Object.keys(rowData)
    for (const prefix of prefixes) {
      const found = keys.find(k => k.toLowerCase().includes(prefix.toLowerCase()))
      if (found && rowData[found] != null) return String(rowData[found]).trim()
    }
    return ''
  }
  ...
  const checkIsDayOff = (dayOffVal: string, dateStr: string) => {
    ...
    if (!y) return false
    const dayName = THAI_DAYS[new Date(y, m - 1, d).getDay()]
    const aliases = DAY_ALIASES[dayName] ?? [dayName]
    return aliases.some(a => dayOffVal.trim().toLowerCase().includes(a.toLowerCase()))
  }
  ...
  const getThaiDayLabel = () => {
    ...
    if (!y) return ''
    return `วัน${THAI_DAYS[new Date(y, m - 1, d).getDay()]}`
  }

  const matchShift = (shiftStr: string) => {
    if (selectedShift === '1') return String(shiftStr).trim() === '1' || String(shiftStr).includes('1')
    if (selectedShift === '2') return String(shiftStr).trim() === '2' || String(shiftStr).includes('2')
    return true
  }

  const allWorkers = workforceRows.map(r => {
    const rd       = r.row_data ?? {}
    const name     = getFieldValue(rd, ['รายชื่อพนักงาน','ชื่อจริง','ชื่อพนักงาน','ชื่อ','name','full_name']) || 'ไม่ระบุชื่อ'
    const dayOff   = getFieldValue(rd, ['วันหยุดประจำสัปดาห์','วันหยุดประจำ','วันหยุด','หยุด','dayoff','day_off','day off'])
    const shiftStr = getFieldValue(rd, ['กะทำงาน','กะ','กะงาน','shift'])
    const isOff    = checkIsDayOff(dayOff, selectedDate)
    const status   = workerStatuses[`${selectedDate}_${selectedStation}_${name}`] ?? (isOff ? 'วันหยุด' : 'ทำงาน')
    return { name, dayOff, shiftStr, isOff, status }
  }).filter(w => matchShift(w.shiftStr))

  const processedWorkers = workforceRows.map((r, i) => {
    const rd       = r.row_data ?? {}
    const name     = getFieldValue(rd, ['รายชื่อพนักงาน','ชื่อจริง','ชื่อพนักงาน','ชื่อ','name','full_name'])
    const nickname = getFieldValue(rd, ['ชื่อเล่น','nickname','nick'])
    const dayOff   = getFieldValue(rd, ['วันหยุดประจำสัปดาห์','วันหยุดประจำ','วันหยุด','หยุด','dayoff','day_off','day off'])
    const shiftStr = getFieldValue(rd, ['กะทำงาน','กะ','กะงาน','shift'])
    const isOff    = checkIsDayOff(dayOff, selectedDate)
    ...
    return { index: i + 1, name: workerName, nickname: nickname || '-', isOff, status, shiftStr }
  }).filter(w => {
    if (searchTerm) {
      const t = searchTerm.toLowerCase()
      if (!w.name.toLowerCase().includes(t) && !w.nickname.toLowerCase().includes(t)) return false
    }
    if (!matchShift(w.shiftStr)) return false
  ...
  const workingCount = allWorkers.filter(w => w.status === 'ทำงาน').length
      ...
      <div className="card space-y-5 md:space-y-6">
          ...
          <div className="flex flex-col sm:flex-row items-center justify-between gap-3 pt-1">
            <div className="flex items-center gap-1 bg-gray-100 p-1 rounded-xl w-full sm:w-auto">
              <button onClick={() => setStatusTab('all')}
                className={`flex-1 sm:flex-none text-xs font-semibold px-3 sm:px-4 py-2 rounded-lg transition-all ${statusTab === 'all' ? 'bg-white text-gray-800 shadow-sm' : 'text-gray-500 hover:text-gray-700'}`}>
                ทั้งหมด ({totalCount})
              </button>
              <button onClick={() => setStatusTab('work')}
                className={`flex-1 sm:flex-none text-xs font-semibold px-3 sm:px-4 py-2 rounded-lg transition-all ${statusTab === 'work' ? 'bg-green-600 text-white shadow-sm' : 'text-green-600 hover:bg-green-50'}`}>
                ทำงาน ({workingCount})
              </button>
              <button onClick={() => setStatusTab('off')}
                className={`flex-1 sm:flex-none text-xs font-semibold px-3 sm:px-4 py-2 rounded-lg transition-all ${statusTab === 'off' ? 'bg-amber-600 text-white shadow-sm' : 'text-amber-600 hover:bg-amber-50'}`}>
                วันหยุด ({dayOffCount})
              </button>
            </div>
        ...
        {loading ? (
          <div className="py-12 space-y-4">
            <div className="flex justify-center"><div className="animate-spin rounded-full h-8 w-8 border-b-2 border-blue-500" /></div>
          ...
          <div className="text-center py-16 border-2 border-dashed border-gray-100 rounded-2xl bg-gray-50/30">
            ...
            <p className="text-xs text-gray-400 mt-1 max-w-sm mx-auto px-4">ยังไม่มีการอัพโหลดแผนเข้างานสำหรับ {stations.find(s => s.value === selectedStation)?.label ?? selectedStation}</p>
          </div>
                      ...
                      <td className="px-2 py-2.5 sm:px-5 sm:py-3 text-center">
                        {w.isOff ? (
                          <span className="inline-flex items-center gap-1 bg-amber-50 text-amber-700 border border-amber-200/60 text-[10px] sm:text-xs px-2 sm:px-2.5 py-0.5 sm:py-1 rounded-full font-medium">
                            <span className="h-1.5 w-1.5 rounded-full bg-amber-500 animate-pulse" />วันหยุด
                          </span>
                        ) : (
                          <span className="inline-flex items-center gap-1 bg-green-50 text-green-700 border border-green-200/60 text-[10px] sm:text-xs px-2 sm:px-2.5 py-0.5 sm:py-1 rounded-full font-medium">
                            <span className="h-1.5 w-1.5 rounded-full bg-green-500" />ทำงาน
                          </span>
                        )}
                      </td>
                      <td className="px-2 py-2.5 sm:px-5 sm:py-3 text-center">
